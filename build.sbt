...
lazy val root = (project in file("."))
  ...
  .settings(
    packageName in Docker := "zio-example",
    dockerUsername in Docker := Some("ksarath"),
    dockerExposedPorts in Docker := Seq(8080),
    organization := "zio.example",
    name := "zio-example",
    maintainer := "sarathksivasankaran@gmail.com",
    licenses := Seq("MIT" -> url(s"https://github.com/ksarath/${name.value}/blob/v${version.value}/LICENSE")),
    scalaVersion := "2.12.8",
    scalacOptions := Seq(
      "-feature",
      "-deprecation",
